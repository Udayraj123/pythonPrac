...
def verify_accuracy(y_inp, y_pre, msk):
  total = 0
  correct = 0
  for i in range(len(y_pre)):
  	if(msk[i // 700] [i % 700 + 50] == 0):
  	  total += 1
      if(y_pre[i] == y_inp[i // 700][i % 700 + 50]):
      	correct += 1
...
if __name__=="__main__":
  ...
  for epoch in range(n_epochs):
    for batch_no in range(2):
      ...
      ans = True
      print("Verifying accuracy : ", verify_accuracy(y_inp, y_predicted, m_inp)
      # debug portion
      # for i in range(3500):
